...
class MainActivity : AppCompatActivity() {
    ...
    private fun onEqualClick() {
        ...
        expression = performMultiplyAndDivide(expression)//乘除
        expression = performAdd(expression) // 加减
        setResultWithScroll(formatResult(expression))
    }

    private fun performMultiplyAndDivide(expression: String) : String {//乘除正则
        return performCalculation(expression, "-?(\\d+)(\\.?\\d*)[×÷]-?(\\d+)(\\.?\\d*)")
    }

    private fun performAdd(expression: String) : String {// 加减正则
        return performCalculation(expression, "-?(\\d+)(\\.?\\d*)\\+-?(\\d+)(\\.?\\d*)")
    }

    private fun performCalculation(expression: String, pattern: String) : String {//正则查找
    ...
    private fun performSingleCalculation(singleExpression : String) : String {//正则运算
    ...
    private fun formatResult(resultString : String) : String {//格式化
